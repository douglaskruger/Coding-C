/* =============================================================================
   ...
   =============================================================================
*/
static const char module_id[] __attribute__((used)) = "$Id: shared_mem_open.c $";
...
/* this define prevents a warning. The gets is part of stdio.h but being decommissioned */
int gets(char * input);
	...
	if (-1 != shm_fd)
	{
		...
		if (attach != MAP_FAILED) /* mmap worked */
		{
			printf("Attached at 0x%lx, first word = 0x%lx\n", 
                (long unsigned int) attach, (long unsigned int)attach);
			if (mprot & PROT_WRITE)
			{
				*((pid_t *)attach) = getpid();
				printf("Set first word to 0x%lx\n",(long unsigned int)attach);
			}
