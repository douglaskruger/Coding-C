...
#define MY_FILE "/home/parallels/test_file.txt"
	...
	*/
	...
	while (1) 
	{
        ...
        */
        ...
        if (strcmp(hostname,"msm2") == 0)
        {
            msg.payload_opcode = 0;
        }
        else 
        { 
            msg.payload_opcode = 5;
        }
        printf(">>> Sending from host:%s G:%s M:%s: Id:%05d Opcode=%02d ", msg.hostname,
            inet_ntoa(s_addr.sin_addr), CLIENT_IP, msg.packet_id, msg.payload_opcode);
        ...
        switch (msg.payload_opcode)
        {
            ...
            case 5:
                sprintf(msg.payload.str, MY_FILE);
                msg.payload_size = strlen(msg.payload.str);
                msg.payload_opcode = 6;
                printf("SERVER: Size=%04d Type=string Message=\"%s\"\n",
                    msg.payload_size, msg.payload.str);
                cnt = sendto(sock, (char *)&msg, sizeof(msg), 0, (struct sockaddr *) &s_addr, addrlen);

                // Open the file and check if any errors opening the file
                FILE * fp = fopen(MY_FILE, "r+w");
                if (fp < 0)
                {
                    printf("Failed to open file \"%s\"", MY_FILE);
                    break;
                }
                printf("SERVER: Write the file - %s with opcode %d\n", MY_FILE, msg.payload_opcode);
                printf("SERVER:  Size=%d Message=\"%s\"\n", (int)strlen(msg.payload.str), 
                    msg.payload.str);

                // Read the buffer while opcode == 6 and there is data
                while((recvfrom(sock, (char *)&msg, sizeof(msg), 0, (struct sockaddr *) &c_addr, 
                    &addrlen) > 0) && (msg.payload_opcode == 6))
                {
                    // Write the buffer to the file
                    printf("SERVER: File Received- Size=%d, Message=\"%s\" file=%s!\n", 
                        (int)strlen(msg.payload.str), msg.payload.str, MY_FILE);
                    write(fileno(fp), msg.payload.str, strlen(msg.payload.str));
                }
                printf("SERVER: Close the file %s\n", MY_FILE);
			...
			&addrlen)) > 0)
        {
            ...
            switch (msg.payload_opcode)
            {
                ...
                case 6:
                    printf("Type=File Requested=%s", msg.payload.str);
                    FILE * fp = fopen(msg.payload.str, "r");
                    if (fp == NULL)
                    {
                        printf("\nFile open failed!\n");
                        break;
                    ...
                    // Read the file and transmit over multicast
                    while (fgets(msg.payload.str, sizeof(msg.payload.str), fp))
                    {
                        msg.packet_id = msg.packet_id + 1;
                        sendto(sock, (char *)&msg, sizeof(msg), 0, (struct sockaddr *) &s_addr, 
                            addrlen);
                        printf("File Transmit- Size=%d, Message=\"%s\" file=%s!\n", 
                            (int)strlen(msg.payload.str), msg.payload.str, MY_FILE);
                        sleep(3);
                    }
                    printf("File Transmit Done- %s!\n", MY_FILE);
                    fclose(fp);
