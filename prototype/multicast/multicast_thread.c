 ...
 * demonstrate :
 * 	1. pthreads - one thread will be server (sends multicast), and one is a client
...
/* Description: 
 * The server thread creates a message and then broadcasts it on the defined multicast group 
 */
void *
server_thread() 
{
	...
	addr.sin_addr.s_addr = inet_addr(mc_addr_str);
	...
	*/
	...
	while (1) 
	{
		...
		*/
		...
		printf(">>> Sending from host:%s G:%s M:%s: Id:%05d Opcode=%02d Size=%02d", 
            msg.hostname, inet_ntoa(addr.sin_addr), CLIENT_IP, msg.packet_id, 
            msg.payload_opcode, msg.payload_size);
...
/* 
 * Description: 
 * The client thread creates a message and then broadcasts it on the defined multicast group 
 */
void *
client_thread()
{
    ...
    */
    ...
    /* Set the timeout to 1 s when receiving a message */
    struct timeval tv;
    tv.tv_sec = 1;
    tv.tv_usec = 0;
	...
	*/
	...
	while (1) 
	{
		cnt = recvfrom(sock, (char *)&msg, sizeof(msg), 0, (struct sockaddr *) &addr, 
			&addrlen);
		if (cnt < 0) 
		{
			perror("recvfrom(): Socket receive timed out.");
            continue;
		} 
		else if (cnt == 0) 
		{
			/* Empty message - break out of loop */
            printf("recvfrom(): Empty receive msg\n");
            continue;
		}
		...
		printf("<<< Received from host:%s G:%s M:%s: Id:%05d Opcode=%02d Size=%03d ", 
            msg.hostname, inet_ntoa(mreq.imr_multiaddr), inet_ntoa(addr.sin_addr), 
...
main(int argc, const char* argv[])
{
	...
	{
		fprintf(stderr, "Main Thread - Error creating server thread\n");
		exit(1);
	}
	...
	{
		fprintf(stderr, "Main Thread - Error creating client thread\n");
		exit(1);
	}
