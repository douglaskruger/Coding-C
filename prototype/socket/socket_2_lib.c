/* =============================================================================
   ...
   =============================================================================
*/
static const char module_id[] __attribute__((used)) = "$Id: socket_2_lib.c $";

/*
 * Description:
 *   This prototype was built from information on the internet including sample code. This was 
 *   done to demonstrate an idea or technology.
 *
 *   This is the communications library
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
conn_t* 
connect_tcp(const char* serverIP, const char* serverPort) 
{
...
conn_t* 
listen_tcp(const char* listenPort) 
{
	int yes=1;
	struct addrinfo hints;
	...
	{
		if ((listenconn->sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) 
		{
			perror("could not create socket with this address, retrying...\n");
			continue;
		}
		if (setsockopt(listenconn->sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1) 
		{
...
int 
recv_tcp(conn_t* connxn, char* buffer, unsigned int bufSize) 
{
...
int 
send_tcp(conn_t* connxn, char* buffer, unsigned int bytesToSend) 
{
	int ret;
	int retVal = 0;

	// all data in the buffer may not be sent on a single call to send(), so loop until all data is sent
...
int 
recvData(conn_t* connxn, char* buffer, unsigned int bufSize) 
{
...
int 
sendData(conn_t* connxn, char* buffer, unsigned int bytesToSend) 
{
